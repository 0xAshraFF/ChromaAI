"""
rs_error_diagnostic.py — Reed-Solomon Error Pattern Analysis
=============================================================
Embeds a 256-bit payload into Band B (DCT[0,2]) of 5 diverse 480p frames
using the original content-adaptive alpha (base=12, multiplier table),
round-trips each through H.265 CRF28 (libx265, ctu=32:min-cu-size=8),
and characterises where and how errors occur.

RS sizing question: are errors bursty (clustered) enough to defeat a naive
consecutive-symbol grouping, or will simple stride interleaving suffice?

Output: printed tables + rs_error_diagnostic_results.json
"""
...
WORK_DIR       = tempfile.mkdtemp(prefix='rs_diag_')
...
def embed(frame: np.ndarray, payload: np.ndarray) -> tuple[np.ndarray, list]:
    """
    Embed `payload` into Band B of a single-channel float64 frame.
    Returns (embedded_uint8, block_alpha_list).
    block_alpha_list[i] = local_alpha used for bit i.
    """
    h, w    = frame.shape
    emb     = frame.copy()
    r, c    = BAND_B
...
# ── H.265 round-trip ──────────────────────────────────────────────────────────
def h265_roundtrip(frame_u8: np.ndarray, crf: int = 28) -> np.ndarray:
    """
    Single-frame H.265 encode/decode.
    Uses ctu=32:min-cu-size=8 per chroma_cascade.py specification.
    Frame must be single-channel uint8 (H, W).
    """
...
def frame_perlin() -> np.ndarray:
    """
    Perlin-like organic texture built from summed octaves of sinusoids
    with random phases — spatially smooth but non-trivial AC energy.
    """
...
def frame_edges() -> np.ndarray:
    """
    Grid of sharp edges every 32 pixels — high-frequency, high AC energy
    concentrated in edge blocks, flat elsewhere.
    """
...
def frame_mixed() -> np.ndarray:
    """Left half: smooth gradient; right half: noise — two texture regimes."""
...
def max_consecutive_run(errors: np.ndarray) -> int:
    """Length of the longest consecutive run of 1s in a binary array."""
    best = cur = 0
    for b in errors:
        if b:
            cur += 1
...
def group_error_counts(errors: np.ndarray, group_size: int = GROUP_SIZE) -> list[int]:
    """
    Split `errors` into consecutive groups of `group_size` and count errors per group.
    Returns list of length N_BITS // group_size.
    """
    n = len(errors) // group_size
    return [int(errors[i*group_size:(i+1)*group_size].sum()) for i in range(n)]


def bit_to_block(bit_idx: int) -> tuple[int, int]:
    """Map bit index → (block_row, block_col) in the raster scan."""
    return divmod(bit_idx, BPR)


def spatial_cluster_score(error_positions: list[int]) -> float:
    """
    Fraction of adjacent-block error pairs among all error pairs.
    High score → errors are spatially clustered (stride interleaving will help).
    Low score  → errors are spatially scattered (permutation better).
    ...
    for i in range(len(blocks)):
        for j in range(i + 1, min(i + 5, len(blocks))):  # check nearby pairs only
            dr = abs(blocks[i][0] - blocks[j][0])
            dc = abs(blocks[i][1] - blocks[j][1])
            if dr + dc <= 2:   # within 2 blocks (Manhattan distance)
                adj += 1
            total += 1
...
def run():
    ...
    print('=' * 72)
    print('Reed-Solomon Error Pattern Diagnostic — H.265 CRF28 (ctu=32:min-cu-size=8)')
    ...
    for name, frame in FRAMES:
        ...
        groups_maj  = sum(1 for g in group_errs if g > GROUP_SIZE // 2)  # >4/8
        ...
        # Spatial error map (block grid, coarse: 1 char per 4×4 block region)
        print(f'\n  Spatial error map (each cell = 4×4 block region, X=any error):')
        map_rows, map_cols = BPC // 4, BPR // 4     # 15 × 26
        ...
        for mr in range(map_rows):
            ...
            for mc in range(map_cols):
                v = err_map[mr, mc]
                row_str += 'X' if v > 0 else '.'
            print(row_str)

        # Alpha at error positions
        if err_pos:
        ...
        frame_summaries.append({
            ...
            'alpha_at_errors': {str(int(k)): v
                                for k, v in (alpha_err_dist.items() if err_pos else {}.items())},
        })
    ...
    print('SUMMARY TABLE')
    print('=' * 72)
    hdr = f'  {"Frame":<16}  {"BRR":>6}  {"Errors":>7}  {"MaxRun":>7}  {"AnyErrGrp":>10}  {"MajCorrGrp":>11}  {"SpatialSc":>10}'
    print(hdr)
    print('-' * 72)
    for s in frame_summaries:
        print(f'  {s["frame"]:<16}  {s["brr"]:>5.1f}%  {s["n_errors"]:>6}/{N_BITS}'
    ...
    if not frame_summaries:
        ...
        # RS(255,k) over GF(2^8): corrects t = (255-k)/2 symbol errors.
        # We have 256 bits = 32 bytes. Use shortened RS with n=32 symbols.
        # t=2 → (32,28): can correct 2 symbol errors / detect 4
        # t=4 → (32,24): can correct 4 symbol errors
        # t=8 → (32,16): can correct 8 symbol errors
        #
        # "Symbol" here = one 8-bit group. An error in any bit of the group
        # corrupts the symbol.
    ...
    # ── Save JSON ─────────────────────────────────────────────────────────────
    out_path = Path(__file__).parent / 'rs_error_diagnostic_results.json'
    out_path.write_text(json.dumps(all_results, indent=2, default=lambda x: int(x) if hasattr(x, 'item') else x))
    print(f'\nSaved → {out_path}')
